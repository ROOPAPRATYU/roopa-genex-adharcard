...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////Users/Satish/Downloads/firsttask/second task/database5.db'
...
class employee(db.Model):
    ...
    emailname=db.Column(db.String(200))
...
def adharform():
    if request.method == "POST":
        ...
        if adhar==None:
            ...
            return render_template("a.html")
    return render_template("adharform.html", login1=email)
...
def emplook():
    if request.method == "POST":
        ...
        if request.form["save"]=="SAVE":
            ...
            emp=employee(compname=cname,joindate=jdate,lastdate=ldate,pic1=pic1.read(),mimetype1=mimetype1,name1=filename,emailname=email)
            db.session.add(emp)
            db.session.commit()
        ...
        if request.form["save"]=="UPDATE":
            ...
            filename = secure_filename(pic1.filename)
            mimetype1=pic1.mimetype
            cname1=employee.query.filter_by(compname=cname,joindate=jdate,lastdate=ldate,emailname=email).first()
            print(cname1)
            if cname1==None:
                print("hello")
                emp1=employee(compname=cname,joindate=jdate,lastdate=ldate,pic1=pic1.read(),mimetype1=mimetype1,name1=filename,emailname=email)
                db.session.add(emp1)
                db.session.commit()
        ...
        if request.form['save']=="EDIT":
            ...
            print(filename)
            print(mimetype1)
            emp=employee.query.filter_by(joindate=jdate,lastdate=ldate,emailname=email).first()
            print(emp)
            if emp is not None:
    ...
    return render_template("emplook.html",employee=employee.query.filter_by(emailname=email).all(),emailname=employee.query.filter_by(emailname=email).first())
...
def alldoc():
    if request.method == "POST":
        ...
        print("hello")
        details=empdoc(e1=e1,e2=e2,e3=e3,epic1=epic1.read(),epic2=epic2.read(),epic3=epic3.read(),ed1=ed1,ed2=ed2,ed3=ed3,edpic1=edpic1.read(),edpic2=edpic2.read(),edpic3=edpic3.read(),pass1=pass1,exdate=exdate,passpic=passpic.read(),pannum=pannum,panpic=panpic.read(),anum=anum,adpic=apic.read(),form16=form16.read(),emailname=email)
        db.session.add(details)
        db.session.commit()
    return render_template("alldoc.html",emailname=employee.query.filter_by(emailname=email).first())
